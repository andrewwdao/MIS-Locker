...
def locker_nowBusy(locker_num, state):
    ...
    if locker_num <= 3:
        if state:  # on
            REG_M1_F |= LOCK_BIT[locker_num+1]  # LOCK0x_BIT
        else:  # off
            REG_M1_F &= ~LOCK_BIT[locker_num+1]  # LOCK0x_BIT
    elif locker_num <= 7:
        if state:  # on
            REG_M1_M |= LOCK_BIT[locker_num+1]  # LOCK0x_BIT
        else:  # off
            REG_M1_M &= ~LOCK_BIT[locker_num+1]  # LOCK0x_BIT
    elif locker_num <= 10:
        if state:  # on
            REG_M1_L |= LOCK_BIT[locker_num+1]  # LOCK0x_BIT
        else:  # off
            REG_M1_L &= ~LOCK_BIT[locker_num+1]  # LOCK0x_BIT
    if locker_num <= 13:
        if state:  # on
            REG_M2_F |= LOCK_BIT[locker_num+1]  # LOCK1x_BIT
        else:  # off
            REG_M2_F &= ~LOCK_BIT[locker_num+1]  # LOCK1x_BIT
    elif locker_num <= 17:
        if state:  # on
            REG_M2_M |= LOCK_BIT[locker_num+1]  # LOCK1x_BIT
        else:  # off
            REG_M2_M &= ~LOCK_BIT[locker_num+1]  # LOCK1x_BIT
    elif locker_num <= 20:
        if state:  # on
            REG_M2_L |= LOCK_BIT[locker_num+1]  # LOCK1x_BIT
        else:  # off
            REG_M2_L &= ~LOCK_BIT[locker_num+1]  # LOCK1x_BIT
    send()
